...
def compute_ycbcr_histogram(img_path: str, values_per_bin: int = 1) -> Tuple[np.ndarray, np.ndarray]:
    """
    Compute YCbCr concatenated histogram from image path.

    Notes:
        OpenCV uses the YCrCb convention internally. We reorder to Y, Cb, Cr when concatenating.
    ...
    hist_y,  bin_edges = compute_histogram(y,  values_per_bin=values_per_bin, density=True)
    hist_cb, _         = compute_histogram(cb, values_per_bin=values_per_bin, density=True)
    hist_cr, _         = compute_histogram(cr, values_per_bin=values_per_bin, density=True)
