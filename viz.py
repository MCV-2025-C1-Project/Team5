...
import math
from src.descriptors import compute_descriptors, grayscale, histogram, hsv, lab, rgb, ycbcr
...
from src.metrics.precision import apk, mapk
...
def generate_comprehensive_analysis(qsd1_dir: str, museum_dir: str, ground_truth_pickle: str,
                                  values_per_bin: int = 1, output_dir: str = "results", k: int = 5,
    ...
    if descriptors:
        print(f"Evaluating descriptors: {descriptors}")
    else:
        print(f"Evaluating all descriptors")

    if distance_metrics:
        print(f"Evaluating distance metrics: {distance_metrics}")
    else:
        print(f"Evaluating all distance metrics")
    ...
    best_system = ComputeImageHistogram(museum_dir, best_dist_func, best_desc_func, values_per_bin)
    ...
    for i, idx in enumerate(sample_indices):
        ...
        plot_top_k_results(str(query_path), retrieved, museum_dir, k=k, save_path=save_path)
    ...
    print("\nGenerating results for all queries...")
    predictions = []
    gt_labels = []
    ...
    print(f"Saved top-{k} results to: {result_pickle_path}")
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/confusion_matrix_top1.png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    accuracy_data = np.array([[sum(top_k_correct), len(top_k_correct) - sum(top_k_correct)]])
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/confusion_matrix_top{k}.png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    print(f"  - Heatmaps: comprehensive_heatmap_map1.png, comprehensive_heatmap_map5.png")
    print(f"  - Sample visualizations: sample_*.png")
    print(f"  - Top-{k} results: result.pkl")
    print(f"  - Confusion matrices: confusion_matrix_top1.png, confusion_matrix_top{k}.png")
    print(f"  - Summary: summary.pkl")
