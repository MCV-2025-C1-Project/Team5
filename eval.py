...
def evaluate_all_descriptors_and_distances(qsd1_dir: str, museum_dir: str,
    ...
    if descriptors is None:
        ...
        for desc in descriptors:
            if desc not in all_descriptors:
                raise ValueError(f"Invalid descriptor: {desc}. Valid options: {all_descriptors}")
    ...
    if distance_metrics is None:
        ...
        for dist in distance_metrics:
            if dist not in all_distance_metrics:
                raise ValueError(f"Invalid distance metric: {dist}. Valid options: {all_distance_metrics}")
    ...
    for desc_idx, descriptor in enumerate(descriptors):
        ...
        for dist_metric in distance_metrics:
            ...
            # Initialize system
            system = ComputeImageHistogram(museum_dir, dist_metric, descriptor, values_per_bin)
